...
//	char *fullpath=".//.SYSSEC//";
      char *vfile=strcat(fullpath,file);
      if (NULL == (fp = fopen(vfile, "rb")))
  //    if (NULL == (fp = fopen(strcat(fullpath,file), "rb")))
      {
            printf("Checksum Unable to open %s for reading\n", vfile);
            return "-1";
      }
...
//size_t vlen=strlen(buf);
    ...
    // int i;
    // for (i = 0; i < 1000000; i++) {
     char* ptr=   md5(msg, len);


     printf("The checksum of %s is %s\n", file,ptr);

return ptr;


 //     return md5(buf, len, 0);
}
...
//#endif
            ...
            d = c;
            c = b;
            sprintf( hashval,"%x + %x + %x + %x, %d", a, f, k[i], w[g], r[i]);
